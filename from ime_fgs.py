...
# System parameters
A = [[1,    0.1],
     [-0.01,  0.99]]  # adjust naming convention for the slice above
B = [[0.],
     [0.01]]
C = [[1, 0]]                  # only the first state was measured
meas_noise_cov = [[1e3]]      # the measurement noise
process_noise_cov = [[0, 0],  # we assume no noise for the process
                     [0, 0]]
...
meas_in_list_msg = [GaussianMeanCovMessage([[m]], [[0]]) for m in meas_list_with_noise]
input_u_list_msg = [GaussianMeanCovMessage([[u]], [[0]]) for u in input_u]
...
meas_noise_node = PriorNode(GaussianMeanCovMessage([[0]], meas_noise_cov), name="N_D")
equality_node = EqualityNode(name="=")

# TODO add the other nodes
B_node = MatrixNode(B, name="B")
input_in_node = PriorNode(name="u_in")
add_input_node = AdditionNode(name="+_input")
process_noise_in_node = PriorNode(GaussianMeanCovMessage([[0], [0]], process_noise_cov), name="N_I")
add_process_noise_node = AdditionNode(name="+_process_noise")
C_node = MatrixNode(C, name="C")
add_meas_noise_node = AdditionNode(name="+_meas_noise")
meas_in_node = PriorNode(name="z_in")
...
# TODO connect the other ports
# Connect input branch: input_in_node -> B_node -> add_input_node
input_in_node.port_a.connect(B_node.port_a)
B_node.port_b.connect(add_input_node.port_b)

# Connect state output and measurement branch
equality_node.ports[1].connect(state_out_node.port_a)
equality_node.ports[2].connect(C_node.port_a)
...
start_state = GaussianMeanCovMessage([[100], [20]], [[99, 0], [0, 99]])
...
for idx, t in enumerate(t_vec):
    ...
    # Forward pass: propagate state through A, add input B*u, add process noise
    A_node.port_b.update(GaussianMeanCovMessage)
    input_in_node.update_prior(input_u_list_msg[idx])
    B_node.port_b.update(GaussianMeanCovMessage)
    add_input_node.port_c.update(GaussianMeanCovMessage)
    add_process_noise_node.port_c.update(GaussianMeanCovMessage)

    # Backward pass: incorporate measurement from meas_in_node back through the graph
    meas_in_node.update_prior(meas_in_list_msg[idx])
    add_meas_noise_node.port_a.update(GaussianWeightedMeanInfoMessage)
    # TODO add the missing updates, pay special attention to the target type and the update order
    C_node.port_a.update(GaussianWeightedMeanInfoMessage)

    # append new estimated state to list
    estimated_state_list.append(equality_node.ports[2].update(GaussianMeanCovMessage))

# Plot result
# Extract results
estimated_position_list = [x.mean[0] for x in estimated_state_list]
estimated_speed_list = [x.mean[1] for x in estimated_state_list]
